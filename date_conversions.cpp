...
long DateConversions::ymd_to_jdn(int y, int m, int d, int julian)
{
    long jdn;
    ...
    if (julian)
            ...
            jdn = (long)(d - 32076) + 1461L * (y + 4800L + (m - 14) / 12) / 4 + 367 * (m - 2 - (m - 14) / 12 * 12) / 12 - 3 * ((y + 4900L + (m - 14) / 12) / 100) / 4 + 1;            /* correction by rdg */

    return jdn;
}


void DateConversions::jdn_to_ymd(long jdn, int *yy, int *mm, int *dd, int julian)
{
    long x, z, m, d, y;
